...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download unlabeled L-PBF layers (IsLabeled=false) from MongoDB GridFS."
    )
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("data_unlabeled"),
        help="Directory to save downloaded unlabeled layers.",
    )
    parser.add_argument(
        "--databases",
        nargs="+",
...
def falsy_filter() -> Dict[str, Dict[str, Sequence]]:
    """IsLabeled가 false인 레이어만 필터링"""
    return {
        "IsLabeled": {"$in": [False, "false", "False", 0, None]},
    }
...
def download_for_db(
    ...
    if debug:
        ...
        sample_doc = collection.find_one(falsy_filter())
        if sample_doc:
            ...
            print(f"[DEBUG] Sample doc IsLabeled: {sample_doc.get('IsLabeled')}")
    ...
    # 비어있는 컬렉션 확인 (빠른 체크)
    sample_count = collection.count_documents(falsy_filter(), limit=1)
    if sample_count == 0:
        print(f"[{db_name}] No unlabeled layers found. Skipping.")
        return result
    
    cursor = collection.find(falsy_filter(), sort=[("LayerNum", 1)])
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    print(f"Filter: unlabeled layers (IsLabeled=false)")
